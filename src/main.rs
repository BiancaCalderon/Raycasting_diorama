...
pub fn cast_ray(
    ...
    // Color de la textura, si está presente
    let mut surface_color = intersect.material.diffuse; // Color por defecto
    if let Some(ref texture) = intersect.material.texture {
        let u = (intersect.point.x - intersect.object.min.x) / (intersect.object.max.x - intersect.object.min.x);
        let v = (intersect.point.y - intersect.object.min.y) / (intersect.object.max.y - intersect.object.min.y);
        
        let texture_color = texture.get_color(u, v);
        surface_color = texture_color; // Usa el color de la textura
    }
...
fn create_cube_grid() -> Vec<Cube> {
    ...
    // Cargar las texturas
    let border_texture = Texture::new("assets/BorderGrass.jpg");
    let inner_texture = Texture::new("assets/InnerGrass.png");
    ...
    let grid_size = 10;
    let cube_size = 0.5;
    let y_offset = -1.0; // Desplazamiento en Y hacia abajo

    // Calcular el desplazamiento para centrar en el eje X
    let offset_x = (grid_size as f32 * cube_size) / 2.0;

    for x in 0..grid_size {
        for z in 0..grid_size {
            let min = Vec3::new(x as f32 * cube_size - offset_x, y_offset, z as f32 * cube_size);
            let max = Vec3::new((x + 1) as f32 * cube_size - offset_x, y_offset + cube_size, (z + 1) as f32 * cube_size);
...
fn main() {
    ...
    let mut camera = Camera::new(
        Vec3::new(0.0, 0.0, 8.5),  // eye: Initial camera position
        Vec3::new(0.0, 0.0, 0.0),  // center: Point the camera is looking at (origin)
        Vec3::new(0.0, 3.0, 0.0)   // up: World up vector
    );
